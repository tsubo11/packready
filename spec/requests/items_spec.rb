...
RSpec.describe "Items", type: :request do
    ...
    describe "POST /packing_lists/:packing_list_id/items" do
        ...
        context '持ち物追加失敗した場合' do
            it 'バリデーションエラーになる' do
                sign_in user
                post "/packing_lists/#{packing_list.id}/items", params: { item: {name:  ' ' , timing:  :day_before }}, headers: { "Accept" => "text/vnd.turbo-stream.html" }
    ...
    describe "PATCH /packing_lists/:packing_list_id/items/:id/check" do
        context 'チェックボックスをクリックした場合' do
            it 'trueに切り替わる' do
                ...
                item.reload
                expect(item.checked).to eq(true)
            end
        end
    ...
    describe "PATCH /packing_lists/:packing_list_id/items/:id" do
        ...
        context '持ち物名の更新が失敗した場合' do
            it 'バリデーションエラーになる' do
                sign_in user
                patch "/packing_lists/#{packing_list.id}/items/#{item.id}", params: { item: {name:  ' ' , timing:  :day_before }}, headers: { "Accept" => "text/vnd.turbo-stream.html" }
                expect(response).to have_http_status(422)
            end
        end
